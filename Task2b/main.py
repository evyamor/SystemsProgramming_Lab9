...
def main(file):
 ...
 with open(file,newline ='') as csvfile:
  ...
  file_temp_c=open("new_temp_file_c",'w')
  file_temp_g=open("new_temp_file_g",'w')
  for row in spamreader:
   file_temp_c.write(row['country']+'\n')
   file_temp_g.write(row['genre']+'\n')
  file_temp_c.close()
  file_temp_g.close()
  calc_c= open('new_temp_file_c','r')
  calc_g= open('new_temp_file_g','r')
  read_data_c= calc_c.read()
  read_data_g= calc_g.read()
  country = set(read_data_c.split())
  genre = set(read_data_g.split())
  fin_arr=[][]
  for word in words:
    for gen in genre:
    fin_arr[i]=word
    fin_arr[i][j]=(genre+","+read_data_g.count(genre))
 with open(file,newline ='') as csvfile:
  spamreader = csv.DictReader(csvfile)
  t_proc= open ('ft','w')
